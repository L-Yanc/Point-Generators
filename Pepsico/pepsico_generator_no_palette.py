...
def generate_points(cols: int, rows: int, aisles: int, start_x: float, final_x: float, start_y: float, 
    ...
    # calculating the angle between the line and the horizontal
    theta = math.atan(m)

    # initializing temp variables and points array
    current_x = start_x + c*2*math.cos(theta) + h_palette/2*math.cos(theta)
    ...
    for col in range(int(cols/2)-1):
        ...
        # generating position
        current_x = current_x + c*math.cos(theta) + h_palette*math.cos(theta)
        current_y = m * current_x + d
